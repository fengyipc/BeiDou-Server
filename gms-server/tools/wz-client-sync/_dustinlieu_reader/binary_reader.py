...
class BinaryReader:
	def __init__(self, reader, key, img_data_base: int = 0):
		self.reader = reader
		self.key = key
		# MapleLib ReadStringBlock(offset + ReadInt32()); offset = WzImage block origin in stream
		self.img_data_base = img_data_base
	...
	def read_wz_uol(self):
		...
		if flag == 0x00 or flag == 0x73:
			...
			rel = self.read_int_32()
			save = self.pos()
			self.seek(self.img_data_base + rel)
