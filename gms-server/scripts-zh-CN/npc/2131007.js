...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (!cm.haveItem(exchangeItem, 100)) {
            ...
            cm.sendGetNumber("嘿，这主意不错！我可以给你 #i4310000#完美音高，每 100 个 #i" + exchangeItem + "##t" + exchangeItem + "# 你给我。你要多少个？（当前持有物品：" + cm.itemQuantity(exchangeItem) + ")", Math.min(300, cm.itemQuantity(exchangeItem) / 100), 1, Math.min(300, cm.itemQuantity(exchangeItem) / 100));
        }
    } else if (status == 2) {
