...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (!cm.haveItem(exchangeItem, 100)) {
            ...
            cm.sendGetNumber("嘿，这主意不错！每100个#i，我就给你#i4310000#完美音调" + exchangeItem + "##t" + exchangeItem + "# 你给我。你想要多少个？（当前持有：" + cm.itemQuantity(exchangeItem) + ")", Math.min(300, cm.itemQuantity(exchangeItem) / 100), 1, Math.min(300, cm.itemQuantity(exchangeItem) / 100));
        }
    } else if (status == 2) {
